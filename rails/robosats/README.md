...
All coordinator communication is routed through `@p2pagos/tor`, which this module installs automatically. No direct Tor/SOCKS dependency is required in the consuming app.
...
| `NUXT_TOR_PROXY_SECRET` | yes | — | Shared secret for the `@p2pagos/tor` proxy |
...
`@p2pagos/tor` is installed automatically — no need to add it separately.
