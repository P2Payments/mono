...
|---------|---------|-------------|
| `@p2pagos/booking` | `/flows/booking`, `/flows/booking/embed` | Booking/scheduling UI — calendar, time slots, optional extras, embeddable iframe variant with custom theming |

## Adding a new flow

1. Create `flows/<name>/` with `package.json`, `module.js`, and `runtime/`
2. Name the package `@p2pagos/<name>` — no `-flow` suffix
...
4. Add `"@p2pagos/<name>": "workspace:*"` to the app's `package.json`
5. Add `'@p2pagos/<name>'` to the app's `nuxt.config.js` modules array
6. Run `pnpm install`
